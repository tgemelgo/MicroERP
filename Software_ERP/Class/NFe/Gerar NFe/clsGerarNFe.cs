...
namespace ERP.NFe
{
    internal class XML_NFe : IDisposable
    {
        ...
        private void Criar_Dados_Tag_det(DataRow row_NF, DataRow row_INF, CriarXML xml)
        {
            ...
            xml.Criar_Campo(sElemento_Prod, "qCom", string.Format("{0:n4}", dValor));

            dValor = Convert.ToDecimal(row_INF["Valor_Unitario"]);
            xml.Criar_Campo(sElemento_Prod, "vUnCom", string.Format("{0:n4}", dValor));

            dValor = Convert.ToDecimal(row_INF["Valor_Total_Item"]);
            xml.Criar_Campo(sElemento_Prod, "vProd", string.Format("{0:n2}", dValor));
            ...
            xml.Criar_Campo(sElemento_Prod, "qTrib", string.Format("{0:n4}", dValor));

            dValor = Convert.ToDecimal(row_INF["Valor_Unitario"]);
            xml.Criar_Campo(sElemento_Prod, "vUnTrib", string.Format("{0:n4}", dValor));
        ...
        private void Criar_Dados_Tag_CSOSN(string sXPath_Elemento, DataRow row_INF, CriarXML xml)
        {
            ...
            {
                case "101":
                    xml.Criar_Campo(sXPath_Elemento, "pCredSN", string.Format("{0:n2}", row_INF["Aliquota_Cred_SN"] == DBNull.Value ? 0 : Convert.ToDecimal(row_INF["Aliquota_Cred_SN"])));
                    xml.Criar_Campo(sXPath_Elemento, "vCredICMSSN", string.Format("{0:n2}", row_INF["Valor_Cred_SN"] == DBNull.Value ? 0 : Convert.ToDecimal(row_INF["Valor_Cred_SN"])));
                    break;
                ...
                case "201":
                    ...
                    if (row_INF["Percentual_Adicionado_Substituicao_Tributaria"] != DBNull.Value)
                        xml.Criar_Campo(sXPath_Elemento, "pMVAST", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Percentual_Adicionado_Substituicao_Tributaria"]))); //-- Percentual adicionado ICMS ST

                    if (row_INF["Percentual_Reducao_Substituicao_Tributaria"] != DBNull.Value)
                        xml.Criar_Campo(sXPath_Elemento, "pRedBCST", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Percentual_Reducao_Substituicao_Tributaria"]))); //-- Percentual de redução ICMS ST

                    xml.Criar_Campo(sXPath_Elemento, "vBCST", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Valor_Base_Substituicao_Tributaria"])));
                    xml.Criar_Campo(sXPath_Elemento, "pICMSST", string.Format("{0:n2}", Convert.ToDecimal(row_INF["aliquota_substituicao_tributaria"])));
                ...
                case "203":
                    ...
                    xml.Criar_Campo(sXPath_Elemento, "vBCST", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Valor_Base_Substituicao_Tributaria"] == DBNull.Value ? 0 : row_INF["Valor_Base_Substituicao_Tributaria"])));
                    xml.Criar_Campo(sXPath_Elemento, "pICMSST", string.Format("{0:n2}", Convert.ToDecimal(row_INF["aliquota_substituicao_tributaria"] == DBNull.Value ? 0 : row_INF["aliquota_substituicao_tributaria"])));
                    xml.Criar_Campo(sXPath_Elemento, "vICMSST", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Valor_Substituicao_Tributaria"] == DBNull.Value ? 0 : row_INF["Valor_Substituicao_Tributaria"])));
                    break;

                case "500":
                    xml.Criar_Campo(sXPath_Elemento, "vBCSTRet", string.Format("{0:n2}", Convert.ToDecimal("0")));
                    xml.Criar_Campo(sXPath_Elemento, "vICMSSTRet", string.Format("{0:n2}", Convert.ToDecimal("0")));
        ...
        private void Criar_Dados_Tag_ICMS(string sXPath_Elemento, DataRow row_INF, CriarXML xml)
        {
            ...
            switch (Convert.ToInt32(row_INF["Tipo_Situacao_Tributaria"]))
            {
                case 0:
                    ...
                    xml.Criar_Campo(sXPath_Elemento, "vBC", string.Format("{0:n2}", Convert.ToDecimal(row_INF["valor_base_icms"])));
                    xml.Criar_Campo(sXPath_Elemento, "pICMS", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Aliquota_ICMS"])));
                    xml.Criar_Campo(sXPath_Elemento, "vICMS", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Valor_ICMS"])));
                    break;

                case 10:
                    xml.Criar_Campo(sXPath_Elemento, "modBC", row_INF["Modalidade_Calculo_ICMS"]);
                    xml.Criar_Campo(sXPath_Elemento, "vBC", string.Format("{0:n2}", row_INF["Valor_Base_ICMS"] == DBNull.Value ? 0 : Convert.ToDecimal(row_INF["Valor_Base_ICMS"])));
                    xml.Criar_Campo(sXPath_Elemento, "pICMS", string.Format("{0:n2}", row_INF["Aliquota_ICMS"] == DBNull.Value ? 0 : Convert.ToDecimal(row_INF["Aliquota_ICMS"])));
                    xml.Criar_Campo(sXPath_Elemento, "vICMS", string.Format("{0:n2}", row_INF["Valor_ICMS"] == DBNull.Value ? 0 : Convert.ToDecimal(row_INF["Valor_ICMS"])));
                    xml.Criar_Campo(sXPath_Elemento, "modBCST", row_INF["Modalidade_Calculo_ICMS_ST"]);
                    if (row_INF["Percentual_Adicionado_Substituicao_Tributaria"] != DBNull.Value)
                        xml.Criar_Campo(sXPath_Elemento, "pMVAST", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Percentual_Adicionado_Substituicao_Tributaria"]))); //-- Percentual adicionado ICMS ST
                    if (row_INF["Percentual_Reducao_Substituicao_Tributaria"] != DBNull.Value)
                ...
                case 60:
                    ...
                    if (row_INF["Valor_Base_Substituicao_Tributaria"] == DBNull.Value)
                        xml.Criar_Campo(sXPath_Elemento, "vBCSTRet", string.Format("{0:n2}", 0));
                    else
                        xml.Criar_Campo(sXPath_Elemento, "vBCSTRet", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Valor_Base_Substituicao_Tributaria"])));

                    if (row_INF["Valor_Base_Substituicao_Tributaria"] == DBNull.Value)
                        xml.Criar_Campo(sXPath_Elemento, "pST", string.Format("{0:n2}", 0));
                    else
                        xml.Criar_Campo(sXPath_Elemento, "pST", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Aliquota_Substituicao_Tributaria"])));

                    xml.Criar_Campo(sXPath_Elemento, "vICMSSubstituto", string.Format("{0:n2}", 0));

                    if (row_INF["Valor_Substituicao_Tributaria"] == DBNull.Value)
                        xml.Criar_Campo(sXPath_Elemento, "vICMSSTRet", string.Format("{0:n2}", 0));
                    else
                        xml.Criar_Campo(sXPath_Elemento, "vICMSSTRet", string.Format("{0:n2}", Convert.ToDecimal(row_INF["Valor_Substituicao_Tributaria"])));
                    break;
        ...
        private void Criar_Dados_Tag_PIS(string sXPath_Elemento, DataRow row_INF, CriarXML xml)
        {
            ...
            if (row_INF["Valor_Base_PIS"] != DBNull.Value)
            {
                ...
                xml.Criar_Campo(sXPath_Elemento, "vBC", string.Format("{0:n2}", dValor));
            }

            if (row_INF["Aliquota_PIS"] != DBNull.Value)
            {
                dValor = Convert.ToDecimal(row_INF["Aliquota_PIS"]);
                xml.Criar_Campo(sXPath_Elemento, "pPIS", string.Format("{0:n2}", dValor));
            }

            if (row_INF["Valor_PIS"] != DBNull.Value)
            {
                dValor = Convert.ToDecimal(row_INF["Valor_PIS"]);
                xml.Criar_Campo(sXPath_Elemento, "vPIS", string.Format("{0:n2}", dValor));
            }
        }
        ...
        private void Criar_Dados_Tag_COFINS(string sXPath_Elemento, DataRow row_INF, CriarXML xml)
        {
            ...
            if (row_INF["Aliquota_COFINS"] != DBNull.Value)
            {
                ...
                xml.Criar_Campo(sXPath_Elemento, "pCOFINS", string.Format("{0:n2}", dValor));
            }

            if (row_INF["Valor_COFINS"] != DBNull.Value)
            {
                dValor = Convert.ToDecimal(row_INF["Valor_COFINS"]);
                xml.Criar_Campo(sXPath_Elemento, "vCOFINS", string.Format("{0:n2}", dValor));
            }
        }
        ...
        private void Criar_Dados_Tag_cobr(DataRow row_NF, CriarXML xml)
        {
            ...
            sb.Append("<vOrig>");
            sb.Append(Convert.ToDecimal(row_NF["Valor_Total_Nota"]).ToString("n2"));
            sb.Append("</vOrig>");

            sb.Append("<vDesc>");
            sb.Append(Convert.ToDecimal(0).ToString("n2"));
            sb.Append("</vDesc>");
